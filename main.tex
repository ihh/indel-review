...
% This essentially describes the evolution of a graph-like structure whose
% edges are subject to the TKF91 model

% Generalization to graph grammars such as pair TAGS (Sakakibara et al, Hickey & Blanchette)

\subsubsection*{Finite-event trajectories and other truncated approximations}
...
% TODO: figure showing a finite-event trajectory
...
% TODO: Although Miklos et al use a reversible model, nothing about their approach requires this
...
as a sum of rate matrices operating at individual sites, using the Kronecker sum $\oplus$
and Kronecker product $\otimes$ to concatenate rate matrices for $k$-mers.
Commuting terms in the matrix exponential can then be reordered systematically.
A similar approach was later used for more general motif evolution models \cite{pmid21827770,pmid26135206}.
\color{black}
It remains to be seen if such an approach can be used for indel models;
such algebraic approaches are certainly very appealing.
...
% TODO: comment that Miklos et al 2004 uses a generalized HMM
...
% TODO: figure showing a Mealy transducer for TKF91 rate matrix (left) and matrix exponential (right)

To ensure the rate matrix satisfies \eqref{DiagonalRates},
we can write $\ratematrix = \offdiagratematrix - \diagmatrix{\offdiagratematrix \vectorofones}$
where $\offdiagratematrix$ is a matrix containing the off-diagonal (and positive) elements of $\ratematrix$,
$\vectorofones$ is a vector of 1's, and $\diagmatrix{\vec{v}}$ is the diagonal matrix whose on-diagonal
elements are given by vector $\vec{v}$.
(In automata theory, $\offdiagratematrix \vectorofones$ is referred to as the ``input projection'' of $\offdiagratematrix$.)

% TODO: briefly review notation of transducer theory
...
\cite{WestessonEtAl2012,BouchardCote2013,IndelHistorian}.   % TODO: UPDATE INDELHISTORIAN REF TO PUBLISHED CITE
% TODO: Cite some other purposes of transducers
...
% TODO: the TKF Structure Tree can be viewed as a context-free grammar or pushdown transducer
...
% TODO: in most of these models, an alignment is assumed fixed and the HMM or grammar used to partition it
% However, in principle, can combine the ability of HMMs/grammars to model indels (and thus specify alignments)
