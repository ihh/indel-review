...
\end{eqnarray}
Furthermore, $\exp(\matr{A} \oplus \matr{B}) = \exp(\matr{A}) \otimes \exp(\matr{B})$.
If $\matr{O}_n \in {\cal K}_n$ is the $N^k \times N^k$ null matrix containing only zeroes,
then the rate matrix $\ratematrix$ for a length-$L$ sequence operated on locally by a context-sensitive rate matrix $\matr{A} \in {\cal K}_m$ can be written as a sum of the form
\[
\ratematrix = \sum_{n=0}^{L-m} \matr{O}_n \oplus \matr{A} \oplus \matr{O}_{L-m-n}
\]
Since $\matr{O}_m \oplus \matr{B}$ commutes with $\matr{A} \oplus \matr{O}_n$,
...
This approach was first used by \cite{LunterHein04} and developed more explicitly, with more in-depth discussion of the associated eigensystem, by \cite{pmid21827770,pmid26135206}.
\color{black}
It remains to be seen if such an approach can be used for indel models.

Again, solutions obtained using truncated approximations will not exactly satisfy \eqref{ChapmanKolmogorov}.
...
In the case of the Kronecker sum expansion of the context-dependent rate matrix, the precision is very good \cite{LunterHein04}.
